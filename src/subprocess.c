...
char ** to_C_array (SEXP _array)
{
  ...
  for (int i=0; i<LENGTH(_array); ++i) {
    const char * element = STRING_ELT(_array, i);
    char * new_element = Calloc(strlen(element) + 1);
...
SEXP C_spawn_process (SEXP _command, SEXP _arguments, SEXP _environment)
{
  ...
  if (!isString(_command) || (LENGTH(_command) != 1) || !strlen(STRING_ELT(_command, 0))) {
  ...
  /* translate into C */
  const char * command = STRING_ELT(_command, 0);
  ...
  SEXP ptr;
  PROTECT(ptr = R_MakeExternalPtr(thisHandle, install("process_handle"), R_NilValue));
...
SEXP C_process_read (SEXP _handle)
{
  SEXP ptr = getAttrib(_handle, "handle_ptr");
  if (ptr == R_NilValue) {
  ...
  process_read(process_handle, buffer, BUFFER_SIZE);
...
SEXP C_process_write (SEXP _handle, SEXP _message)
{
  ...
  const char * message = STRING_ELT(_message, 0);
