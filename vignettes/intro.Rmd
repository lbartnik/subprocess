...
both Linux and Windows, as long as a `ssh` __daemon__ is running on
`localhost` and a correct path to the `ssh` client is passed to
`spawn_process()`.
...
signal to the child process. Linux signals can be listed by looking at the
`signals` variable present in the package. It is constructed automatically
when the package is loaded and its value on __Linux__ is different than on
__Windows__. In the example below we see the list of __Linux__ signals.
